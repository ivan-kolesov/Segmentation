...
class SegmentationController: UIViewController, ARSCNViewDelegate {
    ...
    private func processObjectDetections(for request: VNRequest, error: Error?) {
        ...
        guard let bestObservation = objectObservations.first,
            ...
            delegate?.clasificationOccured(self, identifier: "")
            return
        }
        
        delegate?.clasificationOccured(self, identifier: targetObjectLabel)
